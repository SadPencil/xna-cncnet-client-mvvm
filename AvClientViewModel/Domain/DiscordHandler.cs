...
namespace AvClientViewModel.Domain
{
    ...
    public class DiscordHandler : IDisposable
    {
        ...
        public void Disconnect()
        {
            ...
            Log.Information("DiscordHandler: Disconnected Discord RPC client.");
        }
        ...
        {
            Log.Information($"Discord: Received Ready from user {args.User.Username}");
            client?.SetPresence(CurrentPresence);
        }
        ...
        {
            Log.Information($"Discord: Pipe Connection Established. Valid on pipe #{args.ConnectedPipe}");
        }
        ...
        {
            Log.Information($"Discord: Rich Presence Updated. State: {args.Presence?.State}; Details: {args.Presence?.Details}");
        }

        private void OnSubscribe(object sender, SubscribeMessage args)
        {
            Log.Information($"Discord: Subscribed: {args.Event}");
        }

        private void OnUnsubscribe(object sender, UnsubscribeMessage args)
        {
            Log.Information($"Discord: Unsubscribed: {args.Event}");
        }
