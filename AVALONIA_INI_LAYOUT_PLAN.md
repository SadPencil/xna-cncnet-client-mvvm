...
Create a new service in `AvClientView` that applies INI overrides to Avalonia controls:

```csharp
// AvClientView/Services/IIniLayoutOverlayService.cs
...
**Decision**: Port `Parser.cs` to `AvClientView` as `AvaloniaParser.cs`, operating on `Control` instead of `XNAControl`.
...
**Solution**: Use the existing `CCIniFile` class from `ClientCore` (already migrated to `AvClientViewModel`). It handles both inheritance mechanisms. The INI overlay service just needs to load the file and query sections.
